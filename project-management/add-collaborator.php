...
if ($role_id != 1 && $user_id != $uploader_id) {
    echo json_encode(['status' => 'error', 'message' => 'You are not authorized to add collaborators to this project.']);
    exit();
}
...
if ($check_collab_result->num_rows > 0) {
    echo json_encode(['status' => 'error', 'message' => 'User is already a collaborator for this project.']);
    exit();
}

// Add the collaborator
$add_collab_query = "INSERT INTO `project-collaborators` (proj_ID, user_ID) VALUES (?, ?)";
$add_collab_stmt = $db->prepare($add_collab_query);
$add_collab_stmt->bind_param('si', $proj_ID, $collab_user_ID);

if ($add_collab_stmt->execute()) {
    echo json_encode(['status' => 'success', 'message' => 'Collaborator added successfully.']);
} else {
    echo json_encode(['status' => 'error', 'message' => 'Failed to add collaborator.']);
}
